					...
					if (tileEntityIn.containedWorld.getTileEntity(new BlockPos(unit.x, unit.y, unit.z)) != null) {
						...
						try {
							...
							if (TileEntityRendererDispatcher.instance.getRenderer(renderTE) != null)
								TileEntityRendererDispatcher.instance.getRenderer(renderTE).render(renderTE, partialTicks, matrixStackIn, bufferIn, light, combinedOverlayIn);
...
//							Minecraft.getInstance().getItemRenderer().renderQuads(matrixStackIn,bufferIn.getBuffer(RenderTypeLookup.getRenderType(state)),qds,new ItemStack(Item.getItemFromBlock(state.getBlock())),light, combinedOverlayIn);
						...
						for (BakedQuad bakedquad : qds) {
							...
							if (flag && bakedquad.hasTintIndex())
								i = Minecraft.getInstance().getBlockColors().getColor(state, tileEntityIn.containedWorld, new BlockPos(unit.x, unit.y, unit.z), bakedquad.getTintIndex());
